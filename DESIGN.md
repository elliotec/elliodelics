...
- **Field reports** open with an acid `✺` mark and an elliot.ec-style ledger line — org name, a 2px dotted `ink-faint` leader (thicker than the 1px hairline norm — a line that sweeps in needs enough ink to be watched), mono relationship tag + role (`client · engineering partner`, `employment · director of engineering`) — that hides below 40rem. Reports are ordered by relevance to a buying clinic (vertical client work first, scale proof second, own projects last); the honest relationship tag is printed so the "public record" claim survives the audit it invites.
