...
### Hierarchy
- **Display** (800, `clamp(2.1rem, 0.5rem + 7.2vw, 6rem)`, 0.98): the `.display` class — uppercase, `-0.028em` tracking. Hero H1 ("Code and compliance for the psychedelic industry." — both halves of the practice named at maximum size, compliance as domain noun never as certified adjective; the two-paragraph subhead carries the buyer's moment, then the studio's promise) and the contact headline. The clamp floor is sized so PSYCHEDELIC fits at 320px viewports.
